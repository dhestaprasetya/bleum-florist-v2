...
function openProduct(product, trigger) {
  ...
  document.getElementById('detail-content').innerHTML=`<div class="detail-layout"><img src="assets/images/${product.image}" alt="Foto inspirasi ${product.name}, bukan produk yang dijual"><div class="detail-copy"><p class="eyebrow">${product.category.toUpperCase()} · CONTOH</p><h2 id="detail-title">${product.name}</h2><p class="price">Rp xxx.xxx<small>Harga demo · Konfirmasi harga dan stok ke admin</small></p><p>${product.description}</p><dl><dt>Jenis bunga</dt><dd>${product.type}</dd><dt>Pilihan acara (contoh)</dt><dd>${product.occasions.join(', ')}</dd><dt>Pilihan warna & ukuran</dt><dd>Tanyakan warna, ukuran, dan rangkaian yang tersedia kepada admin.</dd><dt>Stok & waktu pembuatan</dt><dd>Tanyakan stok dan waktu pembuatan lewat WhatsApp.</dd></dl><a class="button" href="${waUrl(product)}" target="_blank" rel="noopener noreferrer">Pesan via WhatsApp ↗</a><p class="data-note">Produk dan foto ini hanya contoh. Tanyakan detail terbaru kepada admin.</p></div></div>`;
